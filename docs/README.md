## Documentation

- **Launch strategy** : [launch-strategy.md](launch-strategy.md) - OSS-first go-to-market, decision gates, 90-day plan (READ THIS FIRST)
- **Product vision** : [vision.md](vision.md) - strategy, positioning, adoption, recommendations
- Product overview : [product.md](product.md) - features, audiences, acceptance criteria
- Technical design : [technical.md](technical.md) - architecture, API, pipeline
- Multi-cloud architecture : [architecture-multicloud.md](architecture-multicloud.md) - provider abstraction, AWS/GCP design
- **Auth architecture** : [architecture-auth.md](architecture-auth.md) - auth vs. data access separation, provider comparison
- **Entra ID setup** : [setup-entra-id.md](setup-entra-id.md) - step-by-step OAuth configuration guide

### Delivery backlog
- [Phase 1 - MVP implemented](backlog/phase-1.md)
- [Phase 2 - Concept validation + smart recommendations](backlog/phase-2.md)
- **[Launch readiness - Pre-launch sprint (~80h)](backlog/launch-readiness.md)** - concrete checklist
- [Phase 3 - Production readiness + adoption engine](backlog/phase-3.md) (post-traction gate)
- [Phase 4 - Multi-cloud and scale](backlog/phase-4.md)

### Cross-phase backlog
- **[Adoption drivers](backlog/adoption-drivers.md)** - product features ranked by adoption ROI
- [AI setup wizard](backlog/ai-setup-wizard.md) - LLM-driven zero-to-dashboard onboarding
- [AI environment analysis](backlog/ai-environment-analysis.md) - LLM-assisted schema mapping
- [AI natural-language queries](backlog/ai-natural-language-queries.md) - ask your data in plain text
- [AI instrumentation assistant](backlog/ai-instrumentation-assistant.md) - LLM-suggested App Insights markers + snippets
