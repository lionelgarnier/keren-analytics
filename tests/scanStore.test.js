...
import { persistScan, getLatestScan, listScans } from "../src/core/scanStore.js";
...
test("persistScan + getLatestScan round-trip", () => {
  ...
  try {
    ...
    const inserted = persistScan(tenantId, payload);
    assert.ok(inserted.id > 0);
    assert.equal(inserted.scannedAt, payload.scannedAt);

    const latest = getLatestScan(tenantId);
...
test("getLatestScan returns null when no scans exist for the tenant", () => {
  const restore = withFreshDb();
  try {
    assert.equal(getLatestScan(freshTenant("none")), null);
  } finally {
    restore();
  }
});
...
test("getLatestScan returns the most recent insert", () => {
  ...
  try {
    ...
    persistScan(tenantId, { scannedAt: "2026-05-01T00:00:00Z", tables: { pageViews: true } });
    persistScan(tenantId, { scannedAt: "2026-05-10T00:00:00Z", tables: { pageViews: true, requests: true } });
    const latest = getLatestScan(tenantId);
...
test("listScans returns scans descending and respects limit", () => {
  ...
  try {
    ...
    for (let i = 0; i < 5; i++) {
      persistScan(tenantId, { scannedAt: `2026-05-0${i + 1}T00:00:00Z`, tables: {} });
    }
    const all = listScans(tenantId);
    assert.equal(all.length, 5);
    // Newest first.
    assert.ok(all[0].id > all[4].id);

    const limited = listScans(tenantId, { limit: 2 });
...
test("persistScan trims history beyond the retention cap", () => {
  ...
  try {
    ...
    const remaining = listScans(tenantId, { limit: 100 });
...
test("persistScan auto-creates the tenant row (no FK violation)", () => {
  ...
  try {
    ...
    const inserted = persistScan(tenantId, { scannedAt: "2026-05-11T00:00:00Z", tables: {} });
    assert.ok(inserted.id > 0);
    assert.ok(getLatestScan(tenantId));
  } finally {
    restore();
  }
});
